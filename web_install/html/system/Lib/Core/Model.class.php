...
class Model {
     ...
     * 分页查询
     * @access public
     * @param int $page_rows 每页记录数
     * @param string $mode 分页模式
     * @return mixed
     */
    public function page($page_rows = 20, $mode = null){
        //克隆当前对象 进行 数据统计
        $model = clone $this;
        ...
        if (empty($mode)) {
            $var_page = C('VAR_PAGE');
            $mode = empty($var_page) ? 'page' : $var_page;
        }
        $this->page = new Pagination($total_rows, $page_rows, $mode);

        return $this->limit($this->page->offset, $this->page->length)->select();
    }

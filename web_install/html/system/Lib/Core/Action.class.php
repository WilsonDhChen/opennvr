...
abstract class Action {
   ...
   /**
    ...
    /**
     * [Extend] Action中输出提示信息 支持Ajax
     * @access protected
     * @param String $status 响应状态  success|error[warn|……]
     * @param String $info 响应信息
	 * @param Mixed $data 响应数据(非ajax下为跳转页面)
     * @return void
     */
	 protected function response($status,$info=NULL,$data=NULL){
		 //一个参数并且是数组情况
		 $arg = func_get_arg(0);
	   ...
	   //
        if($this->isAjax() || IS_AJAX || isset($_GET[C('VAR_JSONP_HANDLER')]) ) {// AJAX提交
            $res           =   array();
            $res['status'] =   $status;
            $res['info']   =   $info;
            $res['data']   =   $data;
			//返回数据格式 检测如果不是jsonp请求 返回默认数据格式
			$type = isset($_GET[C('VAR_JSONP_HANDLER')]) ? 'JSONP' : C('DEFAULT_AJAX_RETURN');
            $this->ajaxReturn($res,$type);
        }else{
			$this->dispatchJump($info,$status=='success'?1:0,$data);
		}
		exit;			 

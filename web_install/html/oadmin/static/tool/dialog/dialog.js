...
dialog.alert = function(content,onclose){
	var onclose =  onclose || $.noop;
	var options = $.extend(true, {},{id:'dialog_alert',content:content,title:'消息提示',ok:$.noop,okValue:'确定',onclose:onclose,fixed:true,skin:'ui-dialog-alert ui-dialog-width'},preset);
	preset = {};
...
dialog.prompt = function(content,ok,value){
	var ok 		= ok || $.noop;
	var value 	= value || '';
	var	content ='<p class="ui-dialog-prompt-p1">'+content+'</p><p class="ui-dialog-prompt-p2"><input text="text" value="'+value+'" class="ui-dialog-value" /></p>';
	var options = $.extend(true, {},{id:'dialog_prompt',content:content,title:'输入提示',onshow:function(){ $('.ui-dialog-value',this.node).select() },ok:function(){ this.value = $('.ui-dialog-value',this.node).val(); return ok.call(this); },okValue:'确定',fixed:true,skin:'ui-dialog-prompt ui-dialog-width',modal:true},preset);
	preset = {};
	return options.modal ? dialog(options).showModal() : dialog(options).show();		
};
// dialog tips
//type[success|error|warning]
dialog.tips = function(type,content,onclose){
	var onclose = onclose || $.noop;
	var options = $.extend(true, {},{id:'dialog_tips',content:'<p class="ui-dialog-tips-'+type+'">'+content+'</p>',padding:10,onshow:function(){ var t = this;setTimeout(function () {t.close().remove();},2000); },onclose:onclose,fixed:true,skin:'ui-dialog-tips'},preset);
	preset = {};
	return options.modal ? dialog(options).showModal() : dialog(options).show();
};
 ...
 * id 弹窗ID
*/
dialog.frame = function (url,title,modal,id){
	var config = {};
...
dialog.loading.show = function(content){
	var content = (typeof content =='string' ? content : false)  || 'loading' ; 
		content+='···';

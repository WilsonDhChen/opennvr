...
(function($){
	...
	$.fn.witaForm = function(custom_options){
		...
		//return
		return {
				setRule:function(field,rule){
					options.rule[field] = $.extend(options.rule[field],rule);	
				},
				getRule:function(field,ruleKey){
					if(!options.rule[field]){
						return undefined;	
					}
					if(ruleKey){
						return options.rule[field][ruleKey];		
					}
					return options.rule[field];	
				}
			};
	...
	function _postForm(the,attrs){
		...
		$(the).submit(function(event){
			...
			if( attrs.enctype == 'multipart/form-data' && typeof(FormData)==='undefined'){
	...
	function _postFormByAjax(the,attrs){
		...
		if(attrs.enctype=='multipart/form-data'){
			ajaxOpt.data		= 	_getFromData(the);
			ajaxOpt.contentType = 	false;
			ajaxOpt.processData = 	false;
		}else{
			ajaxOpt.data		= 	$(the).serialize();	
		}
	...
	//ajax带有文件上传 获取表单信息
	function _getFromData(the){
		
		var data = new FormData(the);
		//非IE的现代浏览器 FormData对象有delete方法
		if('delete' in data){
		
			$(the).find(':file').each(function() {
				var name = $(this).attr('name');
				//原语句 data.delete(name) ,下方这么写 是防止IE10以下浏览器报错,(IE10以下根本就不会执行到这里竟然还TM报错  >﹏< )
				data['delete'].call(data,name);
		...
		//IE10、IE10 FormData对象没有delete方法
		//无法使用FormData创建表单数据，否则无法移除文件
		var iedata = new FormData();
		//添加现有的文件到FormData对象
		$(the).find(':file').each(function() {
			var name = $(this).attr('name');
			var files = $(this).data('files');
			if($.isArray(files)){
				$.each(files,function(index,file){
					if(file){
						iedata.append(name,file);
					}
				})
			}
		});
		//添加非文件域表单字段到FormData对象
		$.each($(the).serializeArray(),function(index,field){
			iedata.append(field.name,field.value);
		});
	...
	function _ruleHandler(rule,the){
		...
		if( rule.key in _rule ){
			var result = _rule[rule.key].call(input,rule.val);
			if(rule.val=== false || result === true ) return true;
			_tipsHandler(rule,input);
			return false;			
		}else{
			$.error('unknown rule : '+rule.key);
			return true;
		}
	...
	function _isInvalid(obj){
		...
		//如果文本类型值为空，直接返回true
		if( $(obj).is(':text,input:hidden,:file,:password,textarea') &&  $(obj).val().length==0 ){
			return false;	
	...
	function _checkAjaxCache(obj,cacheTime){
		...
		//判断是否超过缓存时间
		if( field.time + cacheTime < _timestamp()){
			return false;
		};
	...
	var _rule = {
		...
		regexp:function(val){
			...
			if( $(this).is(':text,input:hidden,:password,textarea') &&  $(this).val().length>0 ){
		...
		required:function(){
			...
			if( $(this).is(':text,:password,input:hidden,textarea') ){
		...
		eqto:function(val){
			...
			if( $(this).is(':text,:password,textarea') && $(this).val().length>0 ){
		...
		selected:function(val){
			...
			if( $(this).is('select') ){
				...
				if( $(this).find("option:selected").length && $.inArray($(this).val().toString(),exclude)===-1 ){
					return true;		
				}
				return false;
		...
		url:function(){
			...
			return /^http(s?):\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"])*$/i.test($(this).val());
		...
		qq:function(){
			...
			return /^[1-9]\d{4,9}$/.test($(this).val());

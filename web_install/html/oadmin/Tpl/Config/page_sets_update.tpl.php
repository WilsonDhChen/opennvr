...
$(function(){
	...
	$('#OAForm').witaForm({
		...
		<?php echo 'rule:{'.($config_rule?'"config_name":{'.$config_rule.'},':'').($identifier_rule?'"identifier_name":{'.$identifier_rule.'},':'').$rules.'"config_sort":{required:true,ptint:true}'."},\n";?>
		submit:function(){
		...
		success:function(response){
			...
			if(response.status=='success'){
				dialog.confirm('修改记录成功，是否返回记录列表?',function(){
        ...
        <div class="oaui-form">
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
	        	...
	        	<tr>
	                ...
	                <td><input type="text" class="oaui-input-text" value="{$detail.identifier}" title="{$identifier_name}" name="identifier_name"  /></td>
	            </tr>                 
                </if>
	        	<tr>
	            	<td width="20%" align="right"><strong>{$config_name|default='记录名称'}</strong>：</td>
	                <td><input type="text" class="oaui-input-text" value="{$detail.name}" title="{$config_name}" name="config_name"  /></td>
	            </tr> 
            	<foreach name="fields" item="field_name" key="field_key">
	        	<tr>
	            	<td width="20%" align="right"><strong>{$field_name}</strong>：</td>
	                <td><input type="text" class="oaui-input-text" value="<?php echo $detail['attrs'][substr($field_key,6)] ?>" title="{$field_name}" name="{$field_key}"  /></td>
	            </tr>
                </foreach>
				<notempty name="config_sort">
	        	<tr>
	            	<td width="20%" align="right"><strong>排序</strong>：</td>
	                <td><input type="text" class="oaui-input-text" value="{$detail.sort}" title="排序" name="config_sort"  /></td>
	            </tr>
				</notempty>
	            <tr>

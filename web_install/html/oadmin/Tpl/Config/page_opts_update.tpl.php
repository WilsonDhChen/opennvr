	...
	$('#OAForm').witaForm({
		...
		<?php 
			if(spower($idname.'__identifier') && !empty($identifier_name)){
				echo 'rule:{'.($config_rule?'"config_name":{'.$config_rule.'},':'').($identifier_rule?'"identifier_name":{'.$identifier_rule.'},':'')."},\n";
			}else{
				echo 'rule:{'.($config_rule?'"config_name":{'.$config_rule.'},':'')."},\n";	
			}
	    ?>		submit:function(){
        ...
        <div class="oaui-form">
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
	        	...
	        	<tr>
	                ...
	                <td><input type="text" class="oaui-input-text" value="{$field_val}" title="{$field_key}" name="field_{$field_key}"  /></td>
	            </tr>
                </foreach>
				<notempty name="config_sort">
				<tr>
					<td width="20%" align="right"><strong>排序</strong>：</td>
					<td><input type="text" class="oaui-input-text" value="{$detail.sort}" title="排序" name="sort"  /></td>
				</tr>
				</notempty>
	            <tr>

...
.formui-file .fieldui-file-uploaded{ position:absolute;height:28px;width:28px; overflow:hidden; text-indent:-999em; display:block;right:-35px; top:0; background:#fff url(__STATIC__/image/formui_uploaded.png) no-repeat center -4px; cursor:pointer; border:none;}
.formui-file .fieldui-file-uploaded:hover{ background-position:center -42px}
/*input:radio*/
...
$(function(){
	...
	//
	$('.formui-file').on('mouseover','.fieldui-file-uploaded',function(event){
		event.stopPropagation();
	}).on('mouseout','.fieldui-file-uploaded',function(event){
		event.stopPropagation();
	})
...
//{//删除已上传文件回调}
function uploadedFileDeleteCallback(field_id){
	var $fileField = $('#field'+field_id);
	var uploaded = parseInt($fileField.data('uploaded')) || 0;
	//{//删除后uploaded的值}
	uploaded = uploaded <= 0 ? 0 : uploaded-1;
	//{//data 存储 删除后uploaded的值}
	$fileField.data('uploaded',uploaded );
	//{//根据file原先required的设定和当前uploaded数量 进行 input:file required属性设置}
	if($fileField.data('required')){
		if(uploaded>0){
			$fileField.find(":file").prop('required',false);
		}else{
			$fileField.find(":file").prop('required',true);	
		}		
	}
	//witaForm file required
	if(!$fileField.data('rule-required')){
		$fileField.data('rule-required',witaForm.getRule($fileField.find(":file").attr('name'),'uploaded'));
	}
	
	if($fileField.data('rule-required')){
		if(uploaded>0){
			witaForm.setRule($fileField.find(":file").attr('name'),{uploaded:false});
		}else{
			witaForm.setRule($fileField.find(":file").attr('name'),{uploaded:true});	
		}		
	}
...
function FormSelectEventChange(select,config_id,from,data){
	var $select = $(select);
	var hasData = $.isArray(data) && data.length>0;
	//如果selectholder 选项option 移除所有子项select
	if(!hasData && $select.val()=='#selectholder#'){
	...
	if(hasData){
		...
		$select.find("option[value='"+(data.shift())+"']").attr('selected',true);
	}
		
	
	$.ajax({
		type:'POST',
		data:{ajax:'select',config_id:config_id,field_value:$select.val(),from : from || '',data: data || []}
		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%" class="formui-list">
        	...
        	<tr class="button-row">
                ...
                <td>
                    ...
                    <input type="submit" class="formui-submit" value="ε修改"></button>
...
function loadFailTips(code){
	dialog.confirm('<p class="ajax-fail-alert"><h3>抱歉，数据加载失败！'+code+'</h3><br>点击【确定】按钮将重新加载页面<br>点击【取消】按钮将返回上一页面</p>',function(){
			window.location.reload();
	},function(){
